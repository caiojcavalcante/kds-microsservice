...
const FEATURED_CATEGORIES = [
    { id: menuData.find(cat => cat.name === "Cortes de carne")?.id, name: "Cortes de carne" },
    { id: menuData.find(cat => cat.name === "Hambúrguer Artesanal")?.id, name: "Hambúrguer Artesanal" },
    { id: menuData.find(cat => cat.name === "Cervejas")?.id, name: "Cervejas" },
    { id: menuData.find(cat => cat.name === "Refrigerantes")?.id, name: "Refrigerantes" },
    { id: menuData.find(cat => cat.name === "Risotos")?.id, name: "Risotos" },
].filter((cat): cat is { id: number; name: string } => cat.id !== undefined);
        ...
        <main className="flex min-h-screen flex-col bg-background overflow-x-hidden">
            ...
            <div className="container mx-auto p-4 space-y-12 pb-12">
                {FEATURED_CATEGORIES.map((category) => {
                    const categoryData = menuData.find(c => c.id === category.id)
